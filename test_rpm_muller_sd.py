...
from cos.rpm import ReplicaPathMethod
...
# String method.
cos_rpm = ReplicaPathMethod(chain_2d)

# Iterate for 150 steps.
niter=150
for _ in range(niter):
    force_ = mbp.get_force(cos_rpm.chain.cartersian_coors)
    cos_rpm.evolve(force_)

    ax.plot(cos_rpm.chain.cartersian_coors[:, 0], cos_rpm.chain.cartersian_coors[:, 1], 'o-', lw=0.5, markersize=0.5, c='grey' )


ax.scatter(cos_rpm.chain.cartersian_coors[:, 0], cos_rpm.chain.cartersian_coors[:, 1], s=0.5, c='r', zorder=5)

plt.savefig('test_rpm_muller_sd.png')
